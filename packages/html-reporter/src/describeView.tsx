...
export const DescribeView: React.FC<React.PropsWithChildren<{
  report: HTMLReport;
  describe?: string;
  tests?: TestCaseSummary[];
  isDescribeExpanded: (describe: string) => boolean;
  setDescribeExpanded: (describe: string, expanded: boolean) => void;
  filter: Filter;
  children?: React.ReactNode
}>> = ({ describe, tests, report, isDescribeExpanded, setDescribeExpanded, filter, children }) => {
  if (!describe || !tests || !tests.length)
    return null;

  const filteredTests = tests.filter(t => t.testId);

  return <Chip
    expanded={isDescribeExpanded(describe)}
    noInsets={true}
    setExpanded={(expanded => setDescribeExpanded(describe, expanded))}
    ...
    {filteredTests.length > 0 && filteredTests.map(test =>
      <div key={`test-${test.testId}`} className={'test-file-test test-file-test-outcome-' + test.outcome}>
        <div style={{ textOverflow: 'ellipsis', overflow: 'hidden' }}>
          ...
          <Link href={`#?testId=${test.testId}`} title={test.title}>
            <span className='test-file-title'>{test.title}</span>
          </Link>
        </div>
        <div className='test-file-details-row'>
          <Link href={`#?testId=${test.testId}`} title={test.title} className='test-file-path-link'>
